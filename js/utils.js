...
function statsHTML(s){
  const items=[{l:"Metrics",v:s.tm,c:"#6366f1"},{l:"Anomalies",v:s.ta,c:"#ea580c"},{l:"Material ↑",v:s.mp,c:"#16a34a"},{l:"Material ↓",v:s.mn,c:"#dc2626"},{l:"Seasonal",v:s.se,c:"#d97706"}];
  if(PLATFORM!=="asset")items.push({l:"🔄",v:s.rc,c:"#a78bfa"});
  return items.map(x=>`<div class="stat-card"><div class="label">${x.l}</div><div class="value" style="color:${x.c}">${x.v}</div></div>`).join("");
}
